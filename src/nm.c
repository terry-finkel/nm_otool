...
symtab (t_ofile *ofile, t_object *object, t_meta *meta, size_t offset) {
	...
	const uint32_t symoff = oswap_32(object, symtab->symoff);
	offset = oswap_32(object, symtab->symoff);

	for (uint32_t k = 0; k < oswap_32(object, symtab->nsyms); k++) {

		const struct nlist *nlist = (struct nlist *)opeek(object, offset, sizeof *nlist);
		if (nlist == NULL) return EXIT_FAILURE; /* ERRNO */

		dprintf(1, "%u: SYM TABLE AT: %u, STR (%u) OFFSET: %u, NLIST OFFSET: %lu, OFILE SIZE: %lu, OBJ SIZE: %lu\n", nlist->n_un.n_strx, symoff, symtab->strsize, stroff, offset, ofile->size, object->size);

//		if (stroff + oswap_32(object, nlist->n_un.n_strx) > object->size) return EXIT_FAILURE; /* ERRNO */
